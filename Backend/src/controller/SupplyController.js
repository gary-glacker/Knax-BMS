...
// @route   GET /api/supply/suppliers
...
// @route   POST /api/supply/suppliers
const CreateSupplier = async (req, res) => {
    try {
        const { name } = req.body;
        if (!name) return res.status(400).json({ message: "Supplier name is required" });
        
        const [result] = await db.query("INSERT INTO `suppliers` (`name`) VALUES (?)", [name]);
...
const CreateSupplyOrder = async (req, res) => {
    try {
        ...
        if (!supplier_id || !order_date || !status) {
            return res.status(400).json({ message: "Please provide supplier_id, order_date, and status" });
        }
...
// @route   GET /api/supply/orders/:id/items
